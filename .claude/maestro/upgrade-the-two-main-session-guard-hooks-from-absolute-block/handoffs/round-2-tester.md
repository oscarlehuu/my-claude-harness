...
The developer reported filesChanged: ["hooks/guard-block-main-edits.sh - added protected-path checks, projected budget gating, budget config parsing, no-count carve-outs, fail-open git handling, and guard logging","hooks/guard-block-main-bash.sh - added protected-path/current-usage budget gating on mutating commands plus config/logging while preserving classification behavior","hooks/test/guard_scratch_test.sh - extended guard tests for budget, protected paths, untracked files, custom config, logs, and relative paths","CLAUDE.md - updated routing prose from absolute delegation to risk/budget-based routing","skills/maestro/SKILL.md - aligned engagement guidance with budget guards and protected paths","README.md - documented maestro budget config, protected globs, and guard logs"].
